...
public class DataSourceConfig {
    ...
    DataSource dataSource(Environment environment) {
        ...
        if (StringUtils.hasText(configuredUrl) && configuredUrl.startsWith("jdbc:sqlite:")) {
            HikariConfig sqliteConfig = new HikariConfig();
            sqliteConfig.setDriverClassName(environment.getProperty("spring.datasource.driver-class-name", "org.sqlite.JDBC"));
            sqliteConfig.setJdbcUrl(configuredUrl);
            sqliteConfig.setMaximumPoolSize(environment.getProperty("spring.datasource.hikari.maximum-pool-size", Integer.class, 1));
            sqliteConfig.setMinimumIdle(environment.getProperty("spring.datasource.hikari.minimum-idle", Integer.class, 1));
            sqliteConfig.setConnectionTimeout(environment.getProperty("spring.datasource.hikari.connection-timeout", Long.class, 30000L));
